...
struct _CMPIConstClass {
   ...
   */
   struct _CMPIInstanceFT {
	 ...
	 CMPIStatus (*addKey)(CMPIInstance* inst, const char *name,
   ...
   */
   struct _CMPIObjectPathFT {
     ...
     unsigned int (*getKeyCount)
	  ...
	  /** Adds/replaces a EPR.
		 @param op ObjectPath this pointer.
         @param eprvalue Epr Value.
       */ 
	CMPIStatus (*addEpr)
              (CMPIObjectPath* op,const char *eprvalue);

	 /** Gets the EPR value contained in this ObjectPath.
	 @param op ObjectPath this pointer.
	 @param rc Output: Service return status (suppressed when NULL).
	 @return Number of properties.
	 */ 
