				      ...
				      CMPIStatus * rc )
{
	...
	if (p) 
	{ 
		return qualifierFT.getDataQualifier ( p->qualifiers, qname, rc ); 
	}
	else
	{
				      ...
				      CMPIStatus * rc )
{
	...
	if (p) 
	{
		return qualifierFT.getDataQualifierAt ( p->qualifiers, index, name, rc );
	}
	else
	{
		CMSetStatus ( rc, CMPI_RC_ERR_NO_SUCH_PROPERTY );
					     ...
					     CMPIStatus * rc )
{
	...
	if (p) 
	{
		return qualifierFT.getQualifierCount ( p->qualifiers, rc );
	}
	else
	{
		CMSetStatus ( rc, CMPI_RC_ERR_NO_SUCH_PROPERTY );
...
static CMPIStatus __ift_addKey ( CMPIInstance * instance,
				 const char * name,
				 CMPIValue * value,
				 CMPIType type )
{
	...
	CMReturn ( ( propertyFT.addProperty ( &i->props,
					      name,
					      type,
					      CMPI_keyValue,
					      value ) )?
