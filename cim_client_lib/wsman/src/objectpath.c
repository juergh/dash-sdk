...
static CMPIStatus __oft_addEpr ( CMPIObjectPath * cop,
				 const char * eprvalue
				 )
{
	struct native_cop * o = (struct native_cop *) cop;

	char * epr = ( eprvalue )? strdup ( eprvalue ): NULL;
...
static CMPIString * __oft_getEpr( CMPIObjectPath * cop,
			         CMPIStatus * rc )
{
	struct native_cop * o = (struct native_cop *) cop;

	return native_new_CMPIString ( o->epr, rc );
}
...
int sameCMPIObjectPath (const CMPIObjectPath *cop1, const CMPIObjectPath *cop2)
{
   ...
   /* Check if epr are the same */ 
   if (strcasecmp(ncop1->epr, ncop2->epr) != 0)
      return 0;
   ...
   *str = 0;
   ...
   if (ns && ns->hdl && *(char*)ns->hdl) {
	  ...
	  strcpy(str,(char*)ns->hdl);
         ...
         for (s=i=0, m=(int)strlen(cns); i<m; i++,s++) {
            if (cns[i]=='/') {
               str[s++]='%';
               str[s++]='2';
               str[s]='F';
...
UtilList *getNameSpaceComponents(CMPIObjectPath * cop)
{
   ...
   if (nss && nss->hdl) {
      ns=(char*)nss->hdl;
      if (ns) for (s=i=0, m=(int)strlen(ns); i<m; i++,s++) {
         if (ns[i]=='/') {
            nsc[s]=0;
