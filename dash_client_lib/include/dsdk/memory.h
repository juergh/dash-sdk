 ...
 **/
#ifndef __DSDK_MEMORY__
#define __DSDK_MEMORY__
...
class CIM_Memory;
class CMemory;
typedef CAPIIterator<CMemory, CIM_Memory> _CMemoryIterator;

/**
  * @class CMemory
  * @brief A class representing a Memory.
  **/
class DSDK_Export CMemory 
{
public:
	DECLARE_API_ITERATOR (CMemory);
	typedef CIM_Memory CIMType;
	
	/**
	 * Enumerates all the memory present under a MAP.
	 * @param client	Pointer to the client interface.
	 * @param cached	true  - Enable caching \n
	 *					false - Disable caching.
	 * @return			Iterator to the memory.
	 */
	static CMemory::iterator enumMemory (IClient* client, bool cached = true);

	/**
	 * Return the underlying CIM object
	 */
	CIM_Memory* getCIMObject (void) const { return _memory; }

public:
	/**
	 * Gets the memory SystemCreationClassName 
	 * @return The memory SystemCreationClassName 
	 */
	string getSystemCreationClassName (void) const;

	/**
	 * Gets the memory SystemName
	 * @return The memory SystemName.
	 */
	string getSystemName (void) const;

	/**
	 * Gets the memory CreationClassName 
	 * @return The memory CreationClassName
	 */
	string getCreationClassName (void) const;

	/**
	 * Gets the memory DeviceID
	 * @return The memory DeviceID.
	 */
	string getDeviceID (void) const;

	/**
	 * Checks if memory is Volatile
	 * @return The vilatile.
	 */
	boolean isVolatile (void) const;
	...
	/**
	 * Gets the memory access type as string Ex (Read/Write etc).
	 * @return The memory access.
	 */
	string getAccessStr (void) const;
	...
	/**
	 * Gets the EnabledState of the memory as string
	 * @return The EnabledState
	 */
	...
	/**
	 * Gets the last RequestedState  of the memory as string
	 * @return The RequestedState 
	 */
	string getRequestedStateStr (void) const;

	/**
	 * Gets the OperationalStatus of the memory
	 * @return The OperationalStatus 
	 */
	vector<uint16> getOperationalStatus (void) const;

	/**
	 * Gets the OperationalStatus  of the memory as string
	 * @return The OperationalStatus 
	 */
	...
	/**
	 * Gets the HealthState of the memory as string
	 * @return The HealthState
	 */
	string getHealthStateStr (void) const;

	/**
	 * Gets the ElementName of the memory
...
public:
	/**
	 * Construct this object from the corresponding CIM_Memory
	 * object
	 */
	CMemory (const CIM_Memory& memory);

	/**
	 * Copy constructor
	 */
	CMemory (const CMemory& memory);
